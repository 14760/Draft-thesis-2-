...
\subsubsection{Methods Fitting the degree distribution - scale free property}
...
A power law distribution may not hold throughout a distribution it is typically present at the tail (high degree) of the degree distribution. There is therefore a minimum value of degree from which the power law distribution holds (there is also frequently dispersion at the end (high) of the distribution. 

The minumum degree is calculated such that it minimises the difference between the empirical distribution and a fitted distribution with the goodness of fit measured by a Kolmogorov Smirnov test in the method described by Clauset and Newman \cite{clauset2009power}. The method is implemented in the poweRlaw package for R \cite{gillespie2015fitting}.

Recent research has suggested that power law distributions may be rare in empirical networks and a log normal distribution may be a better fit. The log normal is still a significant deviation from the Poisson distribution that would be seen in a random graph generated with uniform probability of connection. I have calculated the log likelihood of the log normal, power law and Poisson distribution. As we log likelihood is affected by the number of data points I report the fit of the log normal and Poisson over the same degree range as the power law fit. The log likelihood calculations were carried out using the poweRlaw package for R \cite{gillespie2015fitting} using the \texttt{ll} \textcolor{red}{lookup} \todo{lookup} command.
