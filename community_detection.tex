...
\todo{also the difficulty in chosing the appropriate metric for GO analysis eg p value, fold change, proportion of community}

\section{Clustering methods}

\subsection{Modularity}
See section~\ref{sec:modularity definition}
...
\section{A simple example of spectral clustering}
...
See section~\ref{sec: Spinglass description} also see section in introduction \ref{sec:spinglass from introduction}
\section{Spinglass}
...
see section~\ref{sec:spin glass}
...
This algorithm runs in near linear time but is a heuristic algorithm with no guarantee that a similar clustering will be found. In the implementation in igraph for R and python very disparate values of the modularity are found (\todo{table}) and often the modularity is very low. 
...
\subsection{PSP graph label propagation group results}
...
Counter({0: 471,
         ...
         10: 4,
         11: 3})
\todo{magma gsa for label propagation}
...
\section{OTHER CLUSTERING METHODS GSA}
